...
async function resetWeeklyWants(dryRun = true) {
    ...
    if (dryRun) {
        logger_1.logger.info(`Dry run enabled, would have reset ${weeklyWantPages.length} pages`);
        return;
    }
    const requests = [];
    for (const subpage of weeklyWantPages) {
        logger_1.logger.info(`Resetting ${subpage}`);
        requests.push(wiki.edit(subpage, clearWants, "Reset Weekly Wants"));
    }
    await Promise.all(requests);
    logger_1.logger.info(`Successfully reset ${weeklyWantPages.length} pages`);
    logger_1.logger.info("Purging Category:Villager");
    await wiki.purge("Category:Villager");
    logger_1.logger.info("Purging weekly want sub-pages");
    await wiki.purge(weeklyWantPages.join("|"));
    logger_1.logger.info("Weekly wants reset complete");
}
